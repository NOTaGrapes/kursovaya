...
void __fastcall TmainForm::buttonPaintClick(TObject *Sender)
{
		Series1->Clear();
		Canvas->Brush->Color = clBlack;
		iterFractal = StrToFloat(editIter->Text);
		percFractal = StrToFloat(editPerc->Text);
		Canvas->FillRect(Rect(0,0,sizeFractal,sizeFractal));
		mainFractal.reinit(iterFractal,percFractal,sizeFractal);
		mainFractal.calcStartIreg();
...
void __fastcall TmainForm::buttonResetClick(TObject *Sender)
{
		...
		iterFractal = StrToFloat(editIter->Text);
		percFractal = StrToFloat(editPerc->Text);
        mainFractal.reinit(0,0,sizeFractal);
        PaintBox1->Canvas->Pen->Color = clWhite;
        PaintBox1->Canvas->Pen->Width =1;
		for(int m=0;m!=mainFractal.size();m++)
        {
...
{
	Button1->Caption = mainFractal.boxCount();
}
//---------------------------------------------------------------------------
